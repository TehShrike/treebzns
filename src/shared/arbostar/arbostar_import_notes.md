...
| `project.closed` | any work order whose status name is `Finished` (matched by name — row-level `wo_status_id`s don't line up with the readme's status-tab table), **or the deal is dead**: the project landed on Void (No Go) or Declined Proposal, or every estimate carries row-level status 8 Thinking – No Follow Up Needed / 9 Expired (one live estimate keeps it open; a Declined estimate among all-dead moves it to Declined Proposal instead). Expired/Thinking-only deals stay on Proposal — silence stays distinguishable from an active "no" (decided 2026-08-03). Dying before Work Order never counts as a sale — only that document `represents_billable_sale_when_closed`. One-way on re-imports: the import can close a project but never reopens one closed in-app — so a deal revived in ArboStar after it imported dead needs reopening in-app. An invoice alone does not close the project: ArboStar invoices unfinished work orders and so does this schema (decided 2026-09-12) |
| `project.closed_at` / `project.closed_date` | the closing moment where the export records one: a finished work order's `latest_status_update` (the local day the crew marked it Finished — the latest one when several finished) as local midnight in the derived timezone, or the latest decline's `date_created_unix` for Declined Proposal. `closed_date` is the local day of that instant, the same derivation the `_history` tables use. Void and Expired/Thinking closes have no recorded moment in the export and stay null. Written on insert and on re-import only when the row has no `closed_at` yet, so a date set in-app wins (decided 2026-09-12) |
...
| Default project address | taken from the client row's own address columns (the only address source — ArboStar's profile-only "secondary address" has never had data and is not exported); `client.default_project_address_id` is fixed up after the address rows insert, per the schema's convention |
...
| `latest_status_update` | → `closed_at`/`closed_date` when the work order is `Finished`. MM/DD/YYYY local date of the latest status change. Checked against the September 2026 export: never before the work order's `date_created`, and on finished work orders never before the lead's first invoice date (equal on 723 of 929) |
...
| `client.billing_name` / `.billing_address_line_*` / `.billing_city` / `.billing_state` / `.billing_zip` | no billing-address concept in the export — seeded from the client's name and address at insert, then locally owned (never overwritten on re-import) |
| `client.referred_by` | lead-level `utm_*` data doesn't identify a referrer per client |
| `client_address.client_contact_id` | no per-address contact info in the export — ArboStar contacts belong to the client, not to an address |
