...
| `project.closed` | any work order whose status name is `Finished` (matched by name — row-level `wo_status_id`s don't line up with the readme's status-tab table), or the lead has any invoice (invoicing means the work happened — whether it's *paid* is the billing system's concern, tracked as `payment` rows), **or the deal is dead**: the project landed on Void (No Go), or every estimate carries row-level status 4 Declined / 8 Thinking – No Follow Up Needed / 9 Expired (one live estimate keeps it open). Dead deals keep their document stage — dying at Estimate is a loss you can report on, and only the Work Order document `represents_billable_sale_when_closed`, so closing elsewhere never counts as a sale. One-way on re-imports: the import can close a project but never reopens one closed in-app — so a deal revived in ArboStar after it imported dead needs reopening in-app |
