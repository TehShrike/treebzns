...
const fnDir = join(PROJECT_ROOT, 'src', 'worker', 'server_functions')
...
// Temporal classes are members of the `Temporal` namespace, but the type printer emits them bare
// (e.g. `PlainDate`). Re-qualify them so they resolve against the imported `Temporal` namespace.
const TEMPORAL_TYPE_NAMES = ['Instant', 'PlainDate', 'PlainDateTime', 'PlainTime', 'ZonedDateTime', 'PlainYearMonth', 'PlainMonthDay', 'Duration']
...
const to_client_type = (type_string: string) => {
	const with_db_aliases = type_string.replace(/\bAutogenerated(\w+)\b/g, 'Db$1')
	return TEMPORAL_TYPE_NAMES.reduce(
		(s, name) => s.replace(new RegExp(`(?<!\\.)\\b${name}\\b`, 'g'), `Temporal.${name}`),
		with_db_aliases,
	)
}
...
const type_source_modules = [
	...
	{ specifier: '#shared/type/client.ts', path: ['src', 'shared', 'type', 'client.ts'] },
]
...
const temporal_import = /\bTemporal\./.test(type_entries)
	...
	: ''
const financial_number_import = /\bFinancialNumber\b/.test(type_entries)
...
const outputPath = join(PROJECT_ROOT, 'src', 'client', 'lib', 'server_functions.ts')
...
${temporal_import}${financial_number_import}${validator_import}
