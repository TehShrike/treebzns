...
- [ ] **4. `is_owner`**: `import_employees.ts` sets `is_owner: user.user_type === 'admin'` on insert AND update; notes doc says user_type is dropped and imported employees get `is_owner = 0`. In-app permission — should at minimum leave the update set. Decide insert rule (false vs admin-mapping), then align code + notes
